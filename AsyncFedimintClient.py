...
class AsyncFedimintClient:
    ...
    async def join(self, invite_code: str, set_default: bool = False):
        return await self._post('/admin/join', {"inviteCode": invite_code, "setDefault": set_default})

    class LN:
        def __init__(self, client):
            self.client = client

        async def create_invoice(self, request: LnInvoiceRequest, federation_id: str = None):
            return await self.client._post_with_id('/ln/invoice', request, federation_id)

        async def await_invoice(self, request: AwaitInvoiceRequest, federation_id: str = None):
            return await self.client._post_with_id('/ln/await-invoice', request, federation_id)
        ...
        async def await_pay(self, request: AwaitLnPayRequest, federation_id: str = None):
            return await self.client._post_with_id('/ln/await-pay', request, federation_id)

        async def list_gateways(self):
            return await self.client._get('/ln/list-gateways')

        async def switch_gateway(self, request: SwitchGatewayRequest, federation_id: str = None):
            return await self.client._post_with_id('/ln/switch-gateway', request, federation_id)

    class Wallet:
        def __init__(self, client):
            self.client = client

        async def create_deposit_address(self, request: DepositAddressRequest, federation_id: str = None):
            return self.client._post_with_id('/wallet/deposit-address', data=request, federation_id=federation_id)

        async def await_deposit(self, request: AwaitDepositRequest, federation_id: str = None):
            return await self.client._post_with_id('/wallet/await-deposit', request, federation_id)

        async def withdraw(self, request: WithdrawRequest, federation_id: str = None):
            return await self.client._post_with_id('/wallet/withdraw', request, federation_id)
    ...
    class Mint:
        ...
        async def reissue(self, request: ReissueRequest, federation_id: str = None):
            return await self.client._post_with_id('/mint/reissue', request, federation_id)

        async def spend(self, request: SpendRequest, federation_id: str = None):
            return await self.client._post_with_id('/mint/spend', request, federation_id)

        async def validate(self, request: ValidateRequest, federation_id: str = None):
            return await self.client._post_with_id('/mint/validate', request, federation_id)

        async def split(self, request: SplitRequest, federation_id: str = None):
            return await self.client._post_with_id('/mint/split', request, federation_id)

        async def combine(self, request: CombineRequest, federation_id: str = None):
            return await self.client._post_with_id('/mint/combine', request, federation_id)
